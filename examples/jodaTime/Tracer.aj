...
privileged aspect Tracer {
	...
	pointcut methodEntry() : execution(* org.joda..*.* (..)) &&
		!throwableCreation();

	// all scoped constructors
	pointcut constructor() : call(org.joda..*.new(..)) &&
		!throwableCreation();

	// all scoped object initializers [not sure how to use these yet]
	pointcut objectInitialization() : initialization(org.joda..*.new(..)) && !within(Tracer);

	// all scoped class initializers [not sure how to use these yet]
	pointcut classInitialization() : staticinitialization(org.joda..*.*) && !within(Tracer);
	...
	pointcut libraryEntry() : call(* *.* (..)) &&
		!call(* org.joda..*.* (..)) && // ignore regular methods
	...
	// pointcut lastThing() : execution(void org.joda.LongAnWriter.testWriteCollection()) &&
	// !within(Tracer);
	//
	// after() : lastThing() {
