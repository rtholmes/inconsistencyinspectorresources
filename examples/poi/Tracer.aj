...
privileged aspect Tracer {
	...
	pointcut methodEntry() : execution(* org.apache.poi..*.* (..)) &&
		!execution(java.lang.Object java.lang.Object.clone()) &&
		!throwableCreation();

	// all scoped constructors
	pointcut constructor() : call(org.apache.poi..*.new(..)) &&
		!throwableCreation();

	// all scoped object initializers [not sure how to use these yet]
	pointcut objectInitialization() : initialization(org.apache.poi..*.new(..)) && !within(Tracer);

	// all scoped class initializers [not sure how to use these yet]
	pointcut classInitialization() : staticinitialization(org.apache.poi..*.*) && !within(Tracer);
	...
	pointcut libraryEntry() : call(* *.* (..)) &&
		!call(* org.apache.poi..*.* (..)) && // ignore regular methods
		!call(* edu.washington.cse..*.* (..)) && 		// ignore tracer code
		!call(java.lang.Object java.lang.Object.clone()) &&
		!throwableCreation() &&
		!within(Tracer);
	...
	// pointcut lastThing() : execution(void org.apache.poi.LongAnWriter.testWriteCollection()) &&
	// !within(Tracer);
	//
	// after() : lastThing() {
