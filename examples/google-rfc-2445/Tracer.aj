...
privileged aspect Tracer {
	...
	pointcut methodEntry() : execution(* com.google.ical..*.* (..)) &&
		!throwableCreation();

	// all scoped constructors
	pointcut constructor() : call(com.google.ical..*.new(..)) &&
		!throwableCreation();

	// all scoped object initializers [not sure how to use these yet]
	pointcut objectInitialization() : initialization(com.google.ical..*.new(..)) && !within(Tracer);

	// all scoped class initializers [not sure how to use these yet]
	pointcut classInitialization() : staticinitialization(com.google.ical..*.*) && !within(Tracer);
	...
	pointcut libraryEntry() : call(* *.* (..)) &&
		!call(* com.google.ical..*.* (..)) && // ignore regular methods
	...
	// pointcut lastThing() : execution(void com.google.ical.LongAnWriter.testWriteCollection()) &&
	// !within(Tracer);
	//
	// after() : lastThing() {
