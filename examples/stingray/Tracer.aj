...
privileged aspect Tracer {
	...
	pointcut methodEntry() : execution(* com.XXX_STINGRAY_XXX..*.* (..)) &&
		!throwableCreation();

	// all scoped constructors
	pointcut constructor() : call(com.XXX_STINGRAY_XXX..*.new(..)) &&
		!throwableCreation();

	// all scoped object initializers [not sure how to use these yet]
	pointcut objectInitialization() : initialization(com.XXX_STINGRAY_XXX..*.new(..)) && !within(Tracer);

	// all scoped class initializers [not sure how to use these yet]
	pointcut classInitialization() : staticinitialization(com.XXX_STINGRAY_XXX..*.*) && !within(Tracer);
	...
	pointcut libraryEntry() : call(* *.* (..)) &&
		!call(* com.XXX_STINGRAY_XXX..*.* (..)) && // ignore regular methods
	...
	// pointcut lastThing() : execution(void com.XXX_STINGRAY_XXX.LongAnWriter.testWriteCollection()) &&
	// !within(Tracer);
	//
	// after() : lastThing() {
